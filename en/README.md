...
│   ├── generate_docx.py               │   v1 generation (historical)
│   └── generate_docx_v2.py            │   v2 generation (Phase 6 revision)
│
...
1. **Every phase gets `prompt.md` + `config.json`** — human-and-machine-readable, prevents prompt drift, auditable and re-runnable
...
4. **Review before writing; assembly last** — catch domain errors before drafting begins; the assembler needs full context
...
*English translation: DeepSeek-V4-Pro (via Claude Code CLI)*
