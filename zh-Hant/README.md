...
- 論文正文（`中國上市公司併購重組成功案例研究_v2.md`）存在**三處已知且未修復的缺陷**：商譽口徑矛盾、杜邦分解不自洽、CAR t 值硬編碼。這些缺陷已在文中標註，並在 `項目覆盤歸檔報告.md` 中詳細記錄。
...
│
├── 流水線複用包/                       ← ★ 最有價值的資產
│   ├── 多模型論文流水線_playbook.md    │   方法手冊（五條鐵律+Phase 0-9+量化剖面）
│   ├── 多模型論文流水線_playbook.json  │   機讀版
│   └── 階段模板件.md                   │   prompt+config 參數化骨架
│
├── 數據溯源方案模板.md + .json         ← 四級分類規範 [R][E][S][P]
│
├── 項目覆盤歸檔報告.md + .json         ← 完整項目覆盤（v3.0, CLOSED-FINAL）
├── 起點評估分析.md + .json             ← 方法論反思（四模型+紅隊+鏡像參照）
│
├── 中國上市公司併購重組成功案例研究_v2.md + .json  ← 論文終稿（帶缺陷標註）
│
...
│
├── scripts/                           ← 論文生成腳本
│   ├── generate_docx.py               │   v1 生成（歷史版本）
│   └── generate_docx_v2.py            │   v2 生成（Phase 6 修訂版）
│
└── figures/                           ← 論文圖表
...
1. 讀 `流水線複用包/多模型論文流水線_playbook.md` — 方法手冊，約 25K 字
2. 讀 `項目覆盤歸檔報告.md` — 瞭解這套方法在一篇真實論文上的執行結果
3. 讀 `起點評估分析.md` — 瞭解方法的侷限和反思

### 如果你想複用這套方法

1. 複製 `流水線複用包/` 到你的項目
2. 按 playbook §5 確定你的論文類型，選擇對應的承重階段
3. 打開 `階段模板件.md`，把 `{{佔位符}}` 填成你的選題
...
| 論文體量 | ~22.5K 漢字 / 16 篇文獻 / 7 表 2 圖 | 211 本科畢業論文標準 |
